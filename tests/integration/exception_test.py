...
from stweet.auth import SimpleGuestTokenProvider
...
def test_get_simple_auth_token_with_incorrect_response_1():
    with pytest.raises(RefreshTokenException):
        SimpleGuestTokenProvider().get_new_token(get_client_with_default_response(RequestResponse(400, None)))


def test_get_auth_token_with_incorrect_response_2():
    with pytest.raises(TooManyRequestsException):
        SimpleGuestTokenProvider(50, 150).get_new_token(get_client_with_default_response(RequestResponse(429, None)))


def test_get_auth_token_with_incorrect_response_3():
    with pytest.raises(RefreshTokenException):
        SimpleGuestTokenProvider().get_new_token(get_client_with_default_response(RequestResponse(200, '{}')))


def test_get_auth_token_with_incorrect_response_4():
    with pytest.raises(RefreshTokenException):
        SimpleGuestTokenProvider().get_new_token(get_client_with_default_response(RequestResponse(200, 'LALA')))
